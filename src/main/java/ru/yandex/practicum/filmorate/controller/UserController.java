...
public class UserController {
    ...
    public Optional<User> findById(@PathVariable long userId) {
        ...
        return userService.getUserStorage().getUsers().values().stream().filter(x -> x.getId() == userId).findFirst();
    }

    @GetMapping("/{id}/friends")
    public List<User> findFriendsById(@PathVariable long id) {
        return userService.getUserStorage().getUsers().values().stream().filter(x -> userService.getUserStorage().getUsers().get(id).getFriends().contains(x.getId())).sorted(Comparator.comparing(User::getId)).collect(Collectors.toList());
    }

    @GetMapping("/{id}/friends/common/{otherId}")
    public List<User> findGeneralFriends(@PathVariable(value = "id") Long id, @PathVariable(value = "otherId") Long otherId) {
    ...
    public User dellFriends(@PathVariable(value = "id") Long userId, @PathVariable(value = "friendId") Long friendId) {
